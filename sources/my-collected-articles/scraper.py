...
class MyCollectedArticlesScraper(BaseScraper):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as resp:
                ...
                if og_author and og_author.get("content"):
                    cand_auth = og_author["content"].strip()
                ...
                if date_tag and date_tag.get("content"):
                    raw_dt = date_tag["content"].strip()
                    try:
                        dt = datetime.fromisoformat(raw_dt.replace("Z", "+00:00"))
                        meta["date"] = dt.strftime("%Y-%m-%d")
                        meta["date_str"] = dt.strftime("%-d %B %Y")
                    except Exception:
                        pass
                ...
                if not meta["date"]:
                    ...
                    if time_tag and time_tag.get("datetime"):
                        try:
                            dt = datetime.fromisoformat(time_tag["datetime"].strip().replace("Z", "+00:00"))
                            meta["date"] = dt.strftime("%Y-%m-%d")
                            meta["date_str"] = dt.strftime("%-d %B %Y")
                        except Exception:
                            pass
        except Exception as e:
            print(f"[MyCollectedArticles] Warning: Could not fetch web metadata for {url}: {e}")
    ...
    def parse_inbox_file(self, filepath: str) -> List[Article]:
        ...
        for block in blocks:
            ...
            for raw_url in urls:
                ...
                art_id = f"others-{len(self.articles) + len(parsed_articles) + 1}"
    ...
    def process_all_inboxes(self) -> int:
        ...
        if total_new > 0:
            print(f"[MyCollectedArticles] Ingested {total_new} new articles in total. Rebuilding presentations...")
            os.system(f"python3 {os.path.join(project_root, 'code', 'build.py')}")
